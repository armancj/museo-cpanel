...
interface ToolbarCustomProps {
    ...
    confirmDeleteSelected: (select: NomenclatureAccessConditionsResponse) => void,
    exportExcel: () => void,
    openNew: () => void
}
...
export function ToolbarCustom({ selects, setDialog, confirmDeleteSelected, exportExcel, openNew }: ToolbarCustomProps) {
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <div className="my-2">
                    <Button label="Agregar Condición de acceso " icon="pi pi-plus" severity="success" className=" mr-2" onClick={openNew} />
                    <Button label="Eliminar Condición de acceso" icon="pi pi-trash" severity="danger" onClick={handleDelete} disabled={!selects  || !selects.length} />
                </div>
            </React.Fragment>
        );
    };
