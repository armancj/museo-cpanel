...
import { useCulturalHeritageProperty } from '@/app/(full-page)/cultural-property-heritage/hook/useCulturalHeritageProperty';
import { HeritageHeader } from '@/app/(full-page)/cultural-property-heritage/component/HeritageHeader';
import { creamBg, formatDate, lightBg, primaryColor } from '@/app/(full-page)/cultural-property-heritage/util/culture-function';
...
interface PublicCulturalHeritagePropertyPagProps {
    uuid: string;
}

export default function PublicCulturalHeritagePropertyPag({ uuid }: PublicCulturalHeritagePropertyPagProps) {
    const {
        property,
        loading,
        error,
        ...
        formatBooleanValue,
        handleShare
    } = useCulturalHeritageProperty(uuid);

    const sectionsConfig = useGenerateCulturalRecordSections(extractValue, property, formatExtremeDatesValue, formatDimensionValue, formatQuantityValue, formatArrayValue, formatObjectLocation, formatBooleanValue);
                ...
                <div style={{ padding: '2rem 3rem' }}>
                    <Accordion multiple activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index as number[])} style={{ border: 'none' }}>
                        {sectionsConfig.map((section, sectionIdx) => (
                            <AccordionTab key={sectionIdx} header={<AccordionHeader {...section} primaryColor={primaryColor} />} style={tabStyle}>
